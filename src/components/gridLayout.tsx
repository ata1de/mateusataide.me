    ...
    <div className="grid grid-cols-12 md:grid-rows-12 gap-4 md:max-h-[500px] w-full">
      <div className="col-span-12 md:col-span-5 md:row-span-12 bg-black relative rounded-3xl overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0  bg-opacity-75 text-Black rounded-3xl">
          <h2 className="inline-block w-auto text-xl font-bold leading-tight bg-[#ddd3ab] p-3 rounded-tr-3xl">Top 5: Best Auctions</h2>
          <h2 className="inline-block w-auto text-xl font-bold leading-tight bg-[#ddd3ab] p-3 rounded-tr-3xl">Across Brisbane Last Weekend!</h2>
        </div>
      </div>
      <div className="col-span-12 md:col-span-4 md:row-span-8 flex flex-col justify-between bg-[#b99c56] text-black p-4 rounded-3xl">
        <div>
          <h2 className="text-2xl font-bold leading-9">{t('title')}</h2>
          <p className="my-2 text-sm text-zinc-900">{t('description')}</p>
        </div>
        <Separator className="bg-zinc-900 " />
        <Button className="mt-4 bg-transparent text-black py-2 px-4 rounded-3xl hover:bg-[#cab67a]">{t('button')} <ArrowRight className="ml-auto" size={30}/></Button>
      </div>
      ...
      <div className="col-span-12 p-2 md:col-span-3 md:row-span-2 bg-[#755433]  text-white rounded-3xl flex items-center justify-center">
        <button className="bg-[#755433] hover:underline p-10 text-white py-2 px-4 rounded-3xl flex justify-center items-center gap-3">{t('buttonCategory')} <ArrowRight size={30} className="animate-pulse"/></button>
      </div>
